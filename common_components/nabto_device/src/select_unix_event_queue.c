...
void* queue_thread(void* data)
{
    ...
    while(true) {
        //NABTO_LOG_TRACE(LOG, "Event queue loop running");
        ...
        if (event != NULL) {
            //NABTO_LOG_TRACE(LOG, "About to call cb on standard event %d", event->cb);
        ...
        if (timedEvent != NULL) {
            ...
            //NABTO_LOG_TRACE(LOG, "About to call cb on timed event");
            nabto_device_threads_mutex_lock(queue->mutex);
